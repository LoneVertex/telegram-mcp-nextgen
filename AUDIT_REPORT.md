...
| UP-012 | Medium | Dependency declarations had open ranges and two lockfile ecosystems. | **FIXED/PARTIAL** — `pyproject.toml` and `requirements.txt` now use bounded tested ranges; upstream `uv.lock`/`poetry.lock` provenance remains preserved and should be consolidated before a public release. |
